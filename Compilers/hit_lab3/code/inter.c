...
// Operand func
pOperand newOperand(int kind, int argc, ...) {
    pOperand p = (pOperand)malloc(sizeof(Operand));
    assert(p != NULL);
    p->kind = kind;
    va_list vaList;
    assert(kind >= 0 && kind < OPEnumNum);
...
void deleteOperand(pOperand p) {
    if (p == NULL) return;
    assert(p->kind >= 0 && p->kind < OPEnumNum);
    switch (p->kind) {
        case OP_CONSTANT:
            break;
        case OP_VARIABLE:
...
pInterCode newInterCode(int kind, int argc, ...) {
    ...
    p->kind = kind;
    va_list vaList;
    assert(kind >= 0 && kind < IRCodeEnumNum);
    switch (kind) {
        case IR_LABEL:
        case IR_FUNCTION:
        case IR_GOTO:
...
void deleteInterCode(pInterCode p) {
    assert(p != NULL);
    assert(p->kind >= 0 && p->kind < IRCodeEnumNum);
    switch (p->kind) {
        case IR_LABEL:
        case IR_FUNCTION:
        case IR_GOTO:
...
void printInterCode(FILE *fp, pInterCodeList interCodeList) {
    for (pInterCodes cur = interCodeList->head; cur != NULL; cur = cur->next) {
        assert(cur->code->kind >= 0 && cur->code->kind < IRCodeEnumNum);
        if (fp == NULL) {
            // 输出到终端
...
void addInterCodes(pInterCodeList interCodeList, pInterCodes newCodes) {
    if (interCodeList->head == NULL) {
        interCodeList->head = newCodes;
        interCodeList->cur = newCodes;
    } else {
        interCodeList->cur->next = newCodes;
        newCodes->prev = interCodeList->cur;
        interCodeList->cur = newCodes;
    }
}
...
// printf("In genInterCodes 1\n");
        genInterCodes(node->child);
// printf("In genInterCodes 2\n");
...
void genInterCode(int kind, int argc, ...) {
    ...
    switch (kind) {
        ...
        case IR_WRITE:
            ...
            newCodes = newInterCodes(newInterCode(kind, 1, op1));
            addInterCodes(interCodeList, newCodes);
            break;
        case IR_ASSIGN:
        case IR_GET_ADDR:
        case IR_READ_ADDR:
        ...
        case IR_CALL:
            ...
            if (kind == IR_ASSIGN &&
                ...
                newCodes = newInterCodes(newInterCode(kind, 2, op1, op2));
                addInterCodes(interCodeList, newCodes);
            }
            break;
        case IR_ADD:
        ...
        case IR_DIV:
            ...
            newCodes = newInterCodes(newInterCode(kind, 3, result, op1, op2));
            addInterCodes(interCodeList, newCodes);
            break;
        case IR_DEC:
            // TODO:
            ...
            newCodes = newInterCodes(newInterCode(kind, 2, op1, size));
            addInterCodes(interCodeList, newCodes);
            break;
        case IR_IF_GOTO:
            // TODO:
...
void translateExtDefList(pNode node) {
    ...
    //             | epsilon
// printf("In translateExtDefList 1\n");
    while (node) {
...
void translateExp(pNode node, pOperand place) {
    ...
    if (!strcmp(node->child->name, "LP"))
        ...
        if (strcmp(node->child->sibling->name, "LB") &&
            strcmp(node->child->sibling->name, "DOT")) {
            // Exp -> Exp AND Exp
            //      | Exp OR Exp
            //      | Exp RELOP Exp
            //      | NOT Exp
        ...
        // 数组和结构体访问
            ...
            if (!strcmp(node->child->sibling->name, "LB")) {
                ...
                if (node->child->child->sibling &&
                    ...
                    width = newOperand(
                        OP_CONSTANT, 1, getSize(item->field->type->u.array.elem));
            ...
            // Exp -> Exp DOT ID
                ...
                if (temp->kind == OP_ADDRESS) {
                    target = newOperand(temp->kind, 1, temp->u.name);
                    // target->isAddr = TRUE;
                } else {
                    target = newTemp();
                    genInterCode(IR_GET_ADDR, 2, target, temp);
                }

                pOperand id = newOperand(
                    OP_VARIABLE, 1, newString(node->child->sibling->sibling->val));
                int offset = 0;
                ...
                pOperand tOffset = newOperand(OP_CONSTANT, 1, offset);
                if (place) {
                    ...
                    if (argTemp->op == OP_VARIABLE) {
                        ...
                        if (item && item->field->type->kind == STRUCTURE) {
                            ...
                            pOperand varTempCopy =
                                newOperand(OP_ADDRESS, 1, varTemp->u.name);
...
void translateCond(pNode node, pOperand labelTrue, pOperand labelFalse) {
    ...
    // Exp -> Exp RELOP Exp
        ...
        pOperand relop =
            newOperand(OP_RELOP, 1, newString(node->child->sibling->val));
