...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        # 定义self.corners[0]->左下角 self.corners[1]->左上角 self.corners[2]->右下角 self.corners[3]->右下角
        ...
        self.top = top 
        self.right = right
        # 四个角落都没被访问过置为0; [0]->左下角   [1]->左上角   [2]->右下角   [3]->右下角
        self.corners_vis = [0, 0, 0, 0] 
        # 初始时刻全局状态 (Pacman的起始位置， 四个角的访问状态)
        self.general_start_state = (startingGameState.getPacmanPosition(), [0, 0, 0, 0])
        # 当前的全局状态 [Pacman的位置， 四个角的访问状态]
        self.general_new_state = [startingGameState.getPacmanPosition(), [0, 0, 0, 0]]
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        return self.general_start_state
    ...
    def expand(self, state):
        ...
        """
        '''
        return (child, action, stepCost)
            child:当前位置可以到达的相邻位置坐标和墙的状态    [(x1, y1), [0, 0, 0, 0]]
            action:当前位置到达相邻位置需要做的操作 'North'
            stepCost:对应每个action需要的花费       1
            return [[[(x1, y1), [0, 0, 0, 0]], 'North', 1], [(x2, y2), 'East', 1], ...]
        '''
        children = []
        num = 0
        for action in self.getActions(state):
            ...
            "*** YOUR CODE HERE ***"
            # action是可以运动的方向 E.g. action = 'North'
            # 且函数保证操作合法的，不会撞墙，所以另外两个函数就不用考虑遇到墙的问题了
            next_state = self.getNextState(state, action)
            now2next_cost = self.getActionCost(state, action, next_state)
            children.append([next_state, action, now2next_cost])
    ...
    def getNextState(self, state, action):
        ...
        nextx, nexty = int(x + dx), int(y + dy)
        "*** YOUR CODE HERE ***"
        # 替换NONE用于表示墙角状态： 0->左下角   1->左上角   2->右下角   3->右下角
        is_corner = [0, 0, 0, 0]
        ...
        return ((nextx, nexty), [is_corner[0] or state[1][0], is_corner[1] or state[1][1], is_corner[2] or state[1][2], is_corner[3] or state[1][3]])
...
def cornersHeuristic(state, problem):
    ...
    return h_n
    # return 0 # Default to trivial solution，默认为普通方案
...
def foodHeuristic(state, problem):
    ...
    # print('pos:', position)
    # print('food_grid:', foodGrid)
    # print('food_coor:', food_coordinates)
    "*** YOUR CODE HERE ***"
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        # print(self.food) # self.food 是一个二维地图，T标出了食物
